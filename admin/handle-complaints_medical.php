...
if (isset($_POST['submit_response'])) {
    ...
    // Insert response
    $insert_response = "INSERT INTO admin_responses (message_id, admin_id, admin_name, response, date_sent) 
                         VALUES (?, ?, ?, ?, NOW())";
    ...
    if ($stmt->execute()) {
        ...
        $update_status = "UPDATE admin_messages SET status = 'responded', last_updated = NOW() WHERE message_id = ?";
...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($action == 'mark_read') {
        $update = "UPDATE admin_messages SET status = 'read', last_updated = NOW() WHERE message_id = ?";
        ...
        $update = "UPDATE admin_messages SET status = 'unread', last_updated = NOW() WHERE message_id = ?";
        $stmt = $conn->prepare($update);
        $stmt->bind_param("i", $message_id);
        if ($stmt->execute()) {
...
$count_query = "SELECT COUNT(*) as total FROM admin_messages" . $where_clause;
...
$query = "SELECT * FROM admin_messages" . $where_clause . 
...
if (!empty($message_ids)) {
    ...
    $responses_query = "SELECT * FROM admin_responses WHERE message_id IN ($placeholders) ORDER BY date_sent ASC";
  ...
  <div class="sidebar" id="sidebar">
    <ul class="menu">
      ...
      <li class="menu-item">
        ...
        <a href="handle-complaints_medical.php" class="dropdown-item active" data-page="handle-complaints_medical.php">Handle Complaints From Medical</a>
          <a href="handle-complaints_company.php" class="dropdown-item" data-page="handle-complaints_company.php">Handle Complaints From Company</a>
