...
$user_data = [];
if ($user_id) {
    $stmt = $conn->prepare("SELECT * FROM medical_users WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_profile'])) {
    ...
    if ($valid) {
        $stmt = $conn->prepare("SELECT id FROM medical_users WHERE email = ? AND id != ?");
    ...
    if ($valid) {
        $stmt = $conn->prepare("SELECT id FROM medical_users WHERE license_number = ? AND id != ?");
    ...
    if ($valid) {
        ...
        $stmt = $conn->prepare("UPDATE medical_users SET name = ?, email = ?, license_number = ?, license_document = ? WHERE id = ?");
        ...
        if (!empty($current_password) && !empty($new_password)) {
            if ($new_password != $confirm_password) {
                ...
                $stmt = $conn->prepare("SELECT password FROM medical_users WHERE id = ?");
                $stmt->bind_param("i", $user_id);
                $stmt->execute();
                $result = $stmt->get_result();
                ...
                if (password_verify($current_password, $user['password'])) {
                    ...
                    $stmt = $conn->prepare("UPDATE medical_users SET password = ? WHERE id = ?");
...
<head>
  ...
  <title>Profile Management - BridgeRx Connect</title>
  ...
  <div class="sidebar" id="sidebar">
    <ul class="menu">
      ...
      <li class="menu-item">
        ...
        <div class="dropdown">
          <a href="#" class="dropdown-item active" data-page="profile.php">Profile Management</a>
